...
#### Taxa bar plots ####O2 tress
```{r}
phyloseq_rare%>%
  subset_samples(time %in% c("stress"))->phyloseq_bar
...
p_hist +
   facet_grid(cols=vars(incubation), drop = TRUE, scales = "free_x", space="free")+
  theme(axis.text.x = element_text(size=20, angle=90, vjust=0.2, hjust=1, margin=margin(-8,0,0,0)),
        axis.ticks.x = element_blank(),strip.text = element_text(size  = 20),
        axis.text.y= element_text(size = 20),
        axis.title.y = element_text(size=20),
...
#### Taxa bar plots ####H2O2 tress
```{r}
phyloseq_rare%>%
  subset_samples(time %in% c("stress"))->phyloseq_bar

phyloseq_bar%>%
     subset_samples(stress %in% c("H2O2") )%>%
      subset_samples(stress_level %in% c("control", "median"))%>%
     ps_arrange(time)%>%
      microViz::comp_barplot(
    tax_level = "Genus",
    label = "donor_name",
...
x<-paste0("H2O2med_stress_box.jpeg")
ggsave(filename = x,
       path = "C:/Users/zuendj/Desktop/03_data R/wp2_complex/output",
       width = 34,
       height = 14,
...
clr_diff_h2o2%>%
  ...
  gather(condition, clr, "high":"control")%>%
  subset(condition %in% c("control", "median", "high"))%>%
  mutate(time =factor(time, levels=c("stress", "post_stress_1", "post_stress_2")))%>%
  ggplot(aes(x=time, y=clr))+
  geom_line(aes(colour=donor_name, group=donor_name))+
...
clr_diff_o2%>%
  ...
  ggplot(aes(x=time, y=clr))+
  geom_line(aes(colour=donor_name, group=donor_name))+
  facet_grid(cols=vars(condition, stress_level))
```
