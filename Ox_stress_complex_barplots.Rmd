...
# myPal <- tax_palette(
#   data = phyloseq_rare, rank = "Phylum", n = 10, pal = "greenArmytage",
#   add = c(other = "lightgrey")
# )
# 
# 
# myPal["Bacteroidota"] <- "cyan4"
# myPal["Firmicutes"] <- "indianred"
# myPal["Proteobacteria"] <- "pink3"
# myPal["Actinobacteriota"] <- "#a6cee3"
# 
# tax_palette_plot(myPal)
# 
# phyloseq_rare%>%
#   subset_samples(Sample_id_compete %in% "feces")->phyloseq_controls
...
#     ylab("Proportion") +
#     xlab( "")-> p_hist
# p_hist+ theme(text = element_text(size=15),
#                              axis.ticks.x = element_blank(), legend.position = "bottom",
#               rect = element_rect(fill = "transparent"))-> p_hist
# 
# 
# p_hist
# 
...
ggsave(grid, filename = paste( "O2.jpeg"),
       path = "C:/Users/zuendj/Desktop/03_data R/wp2_complex/output/final",
       width = 35,
       height =22,
       units = c("cm"))
...
p_hist$data%>%
  ...
  ylab("Proportion") +
  scale_y_continuous(breaks=c(0.25, 0.5, 0.75))+
  scale_x_discrete(labels=c( "Pre-culture", "Stress", "Post-stress"))->H2O2_control
...
phyloseq_rare%>%
  subset_samples(time %in% "pre-stress" | 
            (stress_level %in% c("high") & stress %in% c("H2O2") & time %in% c("stress", "post_stress_1")) &
              # (stress_level %in% "max" & stress %in% "O2" & incubation %in% "aerobe" & time %in% c("stress", "post_stress_1")) &
            !time %in% "post_stress_2")->phyloseq_controls
                   # Sample_id_compete %in% "feces"  #with or without feces
...
phyloseq_rare%>%
  subset_samples(time %in% "pre-stress" | 
            (stress_level %in% c("median") & stress %in% c("H2O2") & time %in% c("stress", "post_stress_1")) &
              # (stress_level %in% "max" & stress %in% "O2" & incubation %in% "aerobe" & time %in% c("stress", "post_stress_1")) &
            !time %in% "post_stress_2")->phyloseq_controls
                   # Sample_id_compete %in% "feces"  #with or without feces
...
p_hist$data%>%
  ...
  ylab("Proportion") +
  scale_y_continuous(breaks=c(0.25, 0.5, 0.75))+
  scale_x_discrete(labels=c( "Pre-culture", "Stress", "Post-stress"))->H2O2_median
...
phyloseq_rare%>%
  subset_samples(time %in% "pre-stress" | 
            (stress_level %in% c("low") & stress %in% c("H2O2") & time %in% c("stress", "post_stress_1")) &
              # (stress_level %in% "max" & stress %in% "O2" & incubation %in% "aerobe" & time %in% c("stress", "post_stress_1")) &
            !time %in% "post_stress_2")->phyloseq_controls
                   # Sample_id_compete %in% "feces"  #with or without feces
...
p_hist$data%>%
  ...
  ylab("Proportion") +
  scale_y_continuous(breaks=c(0.25, 0.5, 0.75))+
  scale_x_discrete(labels=c( "Pre-culture", "Stress", "Post-stress"))->H2O2_low
...
filter <- c("Escherichia-Shigella", "Bacteroides", "Anaerostipes")
...
ggsave(filename = paste( filter[i], "O2.png"),
       path = "C:/Users/zuendj/Desktop/03_data R/wp2_complex/output/final/donor_specififc_abund/o2",
       width = 20,
       height = 7.5,
       units = c("cm"))}
...
filter <- c("Erysipelatoclostridium")
...
dendro[,c("OTU", "donor_name", "diff_high")]%>%
  pivot_wider(values_from = diff_high, names_from = OTU)->df_diff
...
left_join(clr_diff_h2o2, df_cutoff[, c("donor_name", "cut_off", "OTU")] )%>%
  ...
  ggplot(aes(y=OTU, x= condition, fill =diff))+
  geom_tile() +
