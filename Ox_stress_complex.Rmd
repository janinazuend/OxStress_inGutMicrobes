...
hplc%>%
 ...
 wilcox_test(ΔButyrate~incubation)%>%
  subset( p < 0.05)
```
##o2 line plots
```{r}
...
do.call(rbind, list(no, ctrl, low, median, high, max, ctrl_a, low_a, median_a, high_a, max_a)) -> combined_df
  ...
  combined_df[, c("donor_name", "time", "stress_level", "stress","incubation", "ΔSuccinate", "ΔFormate", "ΔAcetate","ΔPropionate", "ΔButyrate", "total_C")]%>%
  gather(scfa, conc, "ΔSuccinate":"total_C")->long
...
left_join(setNames(aggregate(long$conc, by=list(long$scfa, long$time, long$stress_level, long$stress, long$incubation), FUN=mean), c("scfa", "time","stress_level", "stress", "incubation", "mean")),
  ...
  mutate(stress_level = factor(stress_level, levels=c("minimal","low", "median", "high", "max")),
         time = factor(time, levels=c("pre-stress" ,"stress", "post_stress_1")),
         scfa = factor(scfa, levels=c("Total mM", "ΔAcetate", "ΔButyrate","ΔPropionate","ΔFormate",  "ΔSuccinate")))%>%
...
left_join(setNames(aggregate(long$conc, by=list(long$scfa, long$time, long$stress_level, long$stress, long$incubation), FUN=mean), c("scfa", "time","stress_level", "stress", "incubation", "mean")),
  ...
  subset(time %in% c("pre-stress" ,"stress", "post_stress_1"))%>%
  subset(scfa %in% c("rel_acetate", "rel_butyrate","rel_propionate"))%>%
...
hplc%>%
  ...
  gather(scfa, conc, "ΔSuccinate":"ΔButyrate")%>%  
  subset(!time %in% c("post_stress_2"))%>%
  subset(scfa %in% c("ΔSuccinate","ΔFormate", "ΔAcetate","ΔPropionate", "ΔButyrate"))->long
...
sub%>%
  ...
  dplyr::select(conc, time, scfa, donor_name) %>%
  group_by( time, donor_name, scfa) %>%
  dplyr::summarize(conc = mean(conc)) %>%
  mutate(scfa = factor(scfa, levels= c("rel_acetate", "rel_butyrate","rel_propionate", 
                              "rel_formate", "rel_succinate", "rel_lactate")))%>%
  ggplot(aes(x=time, y=conc,  fill= scfa , stratum = scfa, alluvium = scfa))+
  ...
  geom_stratum(reverse = FALSE, width = 0.7, color = "transparent") +
  scale_x_discrete(labels = c("Pre-culture", "Stress", "Post-stress"))+
    force_panelsizes(rows = unit(8.5, "cm"), col = unit(1.75, "cm"))  ->hplc_plot
...
hplc%>%
  subset((time %in% "pre-stress" & stress_level %in% "control") | (stress_level %in% "control" & incubation %in% "anaerobe"))%>%
  gather(scfa, conc, "ΔSuccinate":"ΔButyrate")%>%
  subset(scfa %in% c("ΔSuccinate","ΔFormate", "ΔAcetate","ΔPropionate", "ΔButyrate"))%>%
  group_by(scfa)%>%
  t_test(data= .,
...
#   gather(scfa, conc, "rel_succinate":"rel_butyrate")%>%
  hplc_comparison[, c("donor_name", "time", "stress_level", "stress", "ΔSuccinate", "ΔLactate","ΔFormate", "ΔAcetate","ΔPropionate", "ΔButyrate", "total_C")]%>%
  gather(scfa, conc, "ΔSuccinate":"total_C")%>%
...
left_join(setNames(aggregate(long$conc, by=list(long$scfa, long$time, long$stress_level, long$stress), FUN=mean), c("scfa", "time","stress_level", "stress", "mean")),
 ...
 ggplot(aes(x=time, y=mean))+
  ...
  ylab("Metabolite [%]")+
  labs(colour="Donor", fill="Stress level")+
  facet_grid(cols=vars(scfa), rows=vars(stress_level), scales= "free") +
...
# hplc[, c("donor_name", "time", "stress_level", "stress", "rel_formate", "rel_acetate","rel_lactate","rel_propionate","rel_butyrate")]%>%
#   gather(scfa, conc, "rel_formate":"rel_butyrate")-> sub


hplc[, c("donor_name", "time", "stress_level", "stress", "ΔSuccinate", "ΔFormate", "ΔAcetate", "ΔLactate","ΔPropionate", "ΔButyrate", "total_C")]%>%
 gather(scfa, conc, "ΔSuccinate":"total_C")->sub

aggregate(sub$conc, by=list(sub$time, sub$stress_level, sub$stress, sub$scfa), FUN="sd")%>%
  subset(Group.2 %in% c("high", "control"))%>%
  subset(Group.3 %in% "H2O2")%>%
  subset(Group.1 %in% "stress")
  
# hplc[, c("donor_name", "time", "stress_level", "stress", "ΔSuccinate", "ΔFormate", "ΔAcetate","ΔPropionate", "ΔButyrate", "total_C")]%>%
#  gather(scfa, conc, "ΔSuccinate":"total_C")%>%  
...
hplc[, c("donor_name", "time", "stress_level", "stress","ΔButyrate")]%>%
  subset(stress %in% c("H2O2") & stress_level %in% c("control", "median") & !donor_name %in% "D1") %>%
  group_by(time)%>%
  t_test(data= .,
    formula = `ΔButyrate` ~ stress_level, p.adjust.method = "BH")%>%
  add_significance()%>%
  # subset(group2 %in% "high")%>%
  # subset(time %in% "stress")%>%
  subset(p < 0.05)
...
hplc[, c("donor_name", "time", "stress_level", "stress", "incubation","rel_succinate",  "rel_formate", "rel_acetate","rel_propionate","rel_butyrate")]%>%
  gather(scfa, conc, "rel_succinate":"rel_butyrate")%>%
  # hplc[, c("donor_name", "time", "stress_level", "stress", "incubation", "ΔSuccinate", "ΔLactate","ΔFormate", "ΔAcetate","ΔPropionate", "ΔButyrate")]%>%
  # gather(scfa, conc, "ΔSuccinate":"ΔButyrate")%>%
  subset(stress %in% "O2" & 
           stress_level %in% "max")%>%
