...
hplc_raw <- read.xlsx("P:/Shared_documents/Janina_Zuend/08_phd/12_working_package_2/02_hplc_pure/02_H2O2/01_import/20221220_final mic_hplc data_corrected blank_only_knon_metabolites.xlsx")
```
...
HPLC_wide_h2o2%>%
  mutate(total_C = ifelse(total_C == 0, NA, total_C))->HPLC_wide_h2o2_NA

# 
#  HPLC_wide_h2o2 %>%
#    mutate(total_C = ifelse(total_C == 0, NA, total_C))%>%
#    mutate(total_C = ifelse(total_mM == 0, NA, total_mM))->HPLC_wide_h2o2
```

```{r}

  
HPLC_wide_h2o2_NA[, c("assay","condition","butyrate_rel_C", "propionate_rel_C","acetate_rel_C", "formate_rel_C" , "lactate_rel_C", "succinate_rel_C" )]  %>%
  subset(condition %in% "control")%>%
  gather(metabolite, rel_conc, "butyrate_rel_C":"succinate_rel_C")->controls
...
means_controls%>%
  ...
  filter(rel_conc > 0)%>%
  mutate(metabolite =factor(metabolite, levels=c("succinate_rel_C", "lactate_rel_C", "formate_rel_C", "butyrate_rel_C", "propionate_rel_C" ,"acetate_rel_C")))%>%
  ggplot(aes(y=bacteria, x=rel_conc))+
  geom_bar(position="stack", stat="identity", aes(fill=metabolite))+
  theme()+
  facet_grid(rows=vars(Family), scales="free", space="free")+
  scale_fill_manual(values = c( "indianred", "indianred1", "darkgoldenrod2",  "cyan","cyan3","cyan4"))

means_controls%>%
  left_join(., 
            read.xlsx("P:/Shared_documents/Janina_Zuend/08_phd/12_working_package_2/00_experimentation/01_pure_culture/20240207_Strain_panel_metabol.xlsx")[, c("bacteria", "Phylum", "Family", "Genus")])->phylogeny

rbind(phylogeny%>%
        mutate(type= "metabol"), 
      phylogeny %>%
        mutate(metabolite = "b_Family",
               type="a_Phylo"))%>%
  rbind(., phylogeny %>%
        mutate(metabolite = "a_Phylum",
               type="a_Phylo"))%>%
  filter(rel_conc > 0)%>%
  mutate(metabolite = ifelse(metabolite == "propionate_rel_C", "c_propionate_rel_C", metabolite))%>%
  mutate(metabolite = factor(metabolite))->sub


left_join(unique(sub%>%select(bacteria))%>%
            mutate(bacteria = ifelse(bacteria =="Bacteroides dorei", "Phocaeicola dorei", bacteria))%>%
               mutate(bacteria = ifelse(bacteria =="Bacteroides vulgatus", "Phocaeicola vulgatus", bacteria)),
          read.xlsx("P:/Shared_documents/Janina_Zuend/08_phd/12_working_package_2/00_experimentation/01_pure_culture/20240207_Strain_panel_metabol.xlsx")[, c("Genus", "bacteria", "Phylum", "Species", "Family")])%>%
   mutate(Genus=factor(Genus))%>%
  group_by(Genus)%>%
  dplyr::summarise(n = n_distinct(Species))%>%
  mutate(genus = paste0(Genus, " (", n, ")"))->df



setdiff(unique(sub$bacteria), 
...
left_join(sub, df)->all
  
all%>%  
   mutate(genus = factor(genus))%>%
  # mutate(bacteria = reorder(bacteria, Family))%>%
  ggplot(aes(y=genus, x=metabolite))+
  new_scale_fill() +
   geom_tile(data = subset(all, metabolite %in% c("a_Phylum")), aes(fill = Phylum )) +
   scale_fill_manual(values = c(  "#a6cee3","indianred","cyan4","pink3"))+
  guides(fill = guide_legend(nrow = 4))+
  new_scale_fill() +
   # geom_tile(data = subset(sub, metabolite %in% c("b_Family")), aes(fill = Family )) +
   # scale_fill_manual(values=c("#1f78b4","#a6cee3", "#80b1d3","#8dd3c7","#33a02c", "#b2df8a","#e31a1c", "#ff7f00",   "#fb9a99",  "#6a3d9a", "#cab2d6","#fdbf6f", "darkgoldenrod2", "#b15928",  "#fb8072"))+
  new_scale_fill() +
   geom_tile(data = subset(all, metabolite %in% c("succinate_rel_C", "lactate_rel_C", "formate_rel_C", "butyrate_rel_C", "c_propionate_rel_C" ,"acetate_rel_C")), aes(fill = rel_conc )) +
  ...
  scale_x_discrete(labels = c('b_Family' = 'Family', 'a_Phylum' = 'Phylum',"succinate_rel_C" = "Succinate", "lactate_rel_C"="Lactate", "formate_rel_C"="Formate", "butyrate_rel_C"="Butyrate", "c_propionate_rel_C" = "Propionate" ,"acetate_rel_C"="Acetate"))
...
```{r}
# library(ggpubr)
left_join(setNames(aggregate(HPLC_wide_h2o2_NA$total_C, by=list(HPLC_wide_h2o2_NA$condition,  HPLC_wide_h2o2_NA$assay), FUN=sd, na.rm=T), c("condition",  "assay", "sd")),
setNames(aggregate(HPLC_wide_h2o2_NA$total_C, by=list(HPLC_wide_h2o2_NA$condition,  HPLC_wide_h2o2_NA$assay), FUN=mean, na.rm=T), c("condition",  "assay", "mean")))->mean_h2o2
...
HPLC_wide_h2o2[, c("assay", "condition", "total_C")]%>%
  left_join(., setNames(order_h2o2, c("assay", "MIC", "MBC")))%>%
...
mean_h2o2%>%
  ...
  stat_pvalue_manual(stats, label="p.signif", coord.flip = TRUE, vjust=0.5, hide.ns = T,  y.position=90, step.increase = 0.1)+
  facet_grid(rows=vars(assay), scales = "free")+
  coord_flip()+
            xlab("Total C [mM]")-> total_C_h2o2
total_C_h2o2
...
stats_absolute_h2o2%>%
    subset(!is.na(ratio))%>%
    left_join(., setNames(order_h2o2, c("Bacteria", "MIC", "MBC")))%>%
  # mutate(condition = factor(condition, levels=c("control", "NIC-1", "NIC")))%>%
  subset(condition %in% c("control",  "NIC"))%>%
...
plot_grid(diff_metabol_h2o2+theme(legend.position = "none"), 
          total_C_h2o2+theme(legend.position = "none", 
