...
rbind(phylogeny%>%
  ...
  rbind(., phylogeny %>%
        mutate(metabolite = "a_Phylum",
               type="a_Phylo"))%>%
  filter(rel_conc > 0)%>%  mutate(bacteria = ifelse(bacteria == "Prevotella copri", "Segatella copri", bacteria))%>%
  ...
  mutate(metabolite = ifelse(metabolite == "propionate_rel_C", "c_propionate_rel_C", metabolite))->sub
...
setNames(aggregate(HPLC_wide_h2o2_NA$total_C, by=list(HPLC_wide_h2o2_NA$condition,  HPLC_wide_h2o2_NA$assay), FUN=mean, na.rm=T), c("condition",  "assay", "mean"))) %>%
...
left_join(mean_h2o2, stats[, c("assay", "p.signif")])%>%
  left_join(., setNames(order_h2o2, c("assay", "MIC", "MBC")))%>%
  # mutate(condition = factor(condition, levels=c("control", "NIC-1", "NIC")))%>%
  subset(condition %in% c("control",  "NIC"))%>%
  ...
  ylab("Total [mM]")+
  scale_x_discrete(labels=c("a_NIC"="Stress", "control" = "Control"))-> total_C_h2o2


left_join(mean_h2o2[, c("condition", "mean", "assay")]%>%
            subset(condition %in% c("control", "NIC"))%>%
            pivot_wider(names_from = condition, values_from = mean), stats[, c("assay", "p.signif")])%>%
  mutate(ratio = ( `NIC`/ control)*100)%>%
  mutate(p.signif = ifelse(p.signif == "ns", NA, p.signif))%>%
  mutate(assay = ifelse(assay == "Prevotella copri", "Segatella copri", assay))%>%
  mutate(assay = ifelse(assay == "Ruminococcus gnavus", "Mediterraneibacter gnavus", assay))%>%
  ggplot(aes(y=assay, x="Total [mM]", fill=ratio))+
  geom_tile()+
  facet_grid(rows=vars(assay), scales="free", space="free")+
  scale_fill_gradientn(colors = c( "coral", "white", "cyan4"),  na.value= "grey", values = rescale(c(0, 100, 400)), limits=c(0, 400))+
...
plot_grid(
          total_C_h2o2+theme(text=element_text(size=8),
          ...
          diff_metabol_h2o2+theme(text=element_text(size=8),
                                  legend.position = "none",
