...
#Beta Distance between control conditions: H2O2
```{r}
...
##### Additional analysis with additional beta distances
#unifrac - h2o2
```{r}
...
left_join(setNames(aggregate(sub$value, by=list( sub$time, sub$stress_level), FUN=mean), c( "time","stress_level" , "value")), 
          setNames(aggregate(sub$value, by=list( sub$time, sub$stress_level), FUN=sd), c( "time","stress_level" , "sd")))->order_unifrac_h2o2
...
left_join(setNames(aggregate(sub$value, by=list( sub$time, sub$stress_level), FUN=mean), c( "time","stress_level" , "value")), 
          setNames(aggregate(sub$value, by=list( sub$time, sub$stress_level), FUN=sd), c( "time","stress_level" , "sd")))->order_wunifrac_h2o2

```

```{r}

order_unifrac_h2o2$metric <- "UniFrac"
order_wunifrac_h2o2$metric <- "weighted UniFrac"
order$metric <- "Bray-Curtis"


rbind(order, order_unifrac_h2o2, order_wunifrac_h2o2)%>%
  mutate(stress_level = as.character(stress_level))%>%
  mutate(time = factor(time, levels=c("pre-stress", "stress", "post_stress_1")))%>%
  mutate(stress_level = factor (stress_level, levels = c( "low", "median", "high")))%>%
  ggplot(aes(x=time, y=value))+
  geom_line(colour="lightpink3", aes(group = stress_level))+
  facet_grid(cols=vars(stress_level), rows=vars(metric), scale="free")+
  geom_ribbon(aes(ymin = value-sd, ymax = value+sd, group = stress_level), fill="lightpink3",     alpha = 0.3)+
  geom_vline(aes(xintercept = which(levels(time) == 'stress')),  # Specify your desired category label
             linetype = "dashed", color = "coral4", alpha=0.8, size=0.5) +
   geom_vline(aes(xintercept = which(levels(time) == 'post_stress_1')),  # Specify your desired category label
...
ggsave(filename = paste( "h2o2_beta_metrics.jpeg"),
       path = "C:/Users/zuendj/Desktop/03_data R/wp2_complex/output/final",
       width = 12,
       height = 10,
       units = c("cm"))
...
ggsave(filename = paste( "o2_growth_alpha.jpeg"),
       ...
       units = c("cm"))
```
### Beta divesity analysis - additional Beta diversity Metrics
#Distance between control conditions: O2
## Unifraq
```{r}
...
left_join(setNames(aggregate(sub$value, by=list( sub$time, sub$stress_level), FUN=mean), c( "time","stress_level" , "value")), 
          setNames(aggregate(sub$value, by=list( sub$time, sub$stress_level), FUN=sd), c( "time","stress_level" , "sd")))->order_unifrac
...
left_join(setNames(aggregate(sub$value, by=list( sub$time, sub$stress_level), FUN=mean), c( "time","stress_level" , "value")), 
          setNames(aggregate(sub$value, by=list( sub$time, sub$stress_level), FUN=sd), c( "time","stress_level" , "sd")))->order_wunifrac

```

```{r}

order_unifrac$metric <- "UniFrac"
order_wunifrac$metric <- "weighted UniFrac"
order$metric <- "Bray-Curtis"


rbind(order, order_unifrac, order_wunifrac)%>%
  mutate(stress_level = as.character(stress_level))%>%
  mutate(time = factor(time, levels=c("pre-stress", "stress", "post_stress_1")))%>%
  mutate(stress_level = factor (stress_level, levels = c("control", "low", "median", "high", "max")))%>%
  ggplot(aes(x=time, y=value))+
...
ggsave(filename = paste( "o2_beta_metrics.jpeg"),
       path = "C:/Users/zuendj/Desktop/03_data R/wp2_complex/output/final",
       width = 15,
       height = 10,
       units = c("cm"))
