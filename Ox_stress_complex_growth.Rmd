...
plot_grid(h2o2_OD + theme(axis.ticks.x =  element_blank(), 
            ...
            force_panelsizes(rows =unit(1.75, "cm")),
...
hplc%>%
  ...
  add_significance()%>%
  add_xy_position()%>%
  mutate(xmax = ifelse(stress_level == "control", 1.2, xmax),
         xmin = ifelse(stress_level == "control", 0.8, xmin))%>%
  mutate(xmax = ifelse(stress_level == "low", 1.8, xmax),
         xmin = ifelse(stress_level == "low", 2.2, xmin))%>%
  mutate(xmax = ifelse(stress_level == "median", 3.2, xmax),
         xmin = ifelse(stress_level == "median", 2.8, xmin))%>%
  mutate(xmax = ifelse(stress_level == "high", 4.2, xmax),
         xmin = ifelse(stress_level == "high", 3.8, xmin))->stat_test
