...
```
###aerobic and anaerobic values go next to each other in order to do correlation
```{r}
setwd("P:/Shared_documents/Janina Zünd/08_phd/09_student thesis/Marina/03_experimentation/03_MIC_MBC/02_hplc results/Single_bacteria_HPLC_blank_corrected - Copy")
...
##create subset
  subset<- HPLC_values_blank_corrected_names$row_names == bacteria_names[i]
  bacteria_subset<-HPLC_values_blank_corrected_names[subset,]

  t<-paste0(bacteria_names[i])

  p_individual<-ggplot(subset(bacteria_subset, metabolite %in% c("succinate", "lactate", "formate", "acetate", "propionate", "butyrate")), aes(x=metabolite, y=concentration, fill=condition)) +
    stat_summary(fun = mean, geom = "bar", position = position_dodge(1), color = "black") +
    stat_summary(fun.data = mean_se, geom = "errorbar" , width = .5, position = position_dodge(1)) +
    #scale_fill_manual(values=c('darkgoldenrod1', "cyan4"))+
    facet_grid( rows=vars(condition_agar), cols= vars(metabolite), scales = "free", switch="y")+
...
#drop ethanol
HPLC_values<-HPLC_values_wide[,-c(9,11,12,13)]
...
for (i in 1:nrow(HPLC_values)){
  x<-HPLC_values[i,c(4:9)]
  HPLC_values[i,10]<- sum(x[!is.na(x) & x > 0], na.rm=T)
  
  for (j in 4:9){
    HPLC_values_relative_wide[i,j] = HPLC_values[i,j] / HPLC_values[i, 10]
  }
}
```
...
means_corr<-join(anaerobe_means, aerobe_means)
```
...
ggplot(selection, aes(x=metabolite, y=Bacteria, fill=ratio))+
  geom_tile()+
  scale_fill_gradientn(colors = c("coral", "coral", "white", "cyan4"),  values = rescale(c(-0.6, -0.5,0,0.37)))+
  facet_grid(rows=vars(Class), space="free", scale="free")+
    theme(strip.text.y = element_text(angle=0, size = 15),
        axis.title = element_blank(),
        axis.text.x = element_text(angle = 90, size=15, vjust=0.2),
