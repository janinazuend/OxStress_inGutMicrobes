...
MBC_MIC_rel_long_label%>%
  subset(bacteria %in% "Bacteroides xylanisolvens" & condition %in% 10)
...
```{r}
hoi$assay<-factor(hoi$assay, levels=c("MIC", "MBC"))
...
ggsave(filename = "MIC_MBC_h2o2_growth_only.jpeg", 
...
tax<-left_join(hoi, taxonomy)


tax %>%
  subset(condition %in% c(9,10,11))%>%
  subset(assay %in% c("MIC"))%>%
    mutate(bacteria = recode(bacteria, "Eubacterium eligens" = "Lachnospira eligens"))->sub
...
sub%>%
  ...
  theme(text = element_text(size=15), 
        ...
        panel.background = element_rect(fill="white"),
        panel.border=element_rect(fill=NA))+
  facet_grid(rows =vars(Family),  cols=vars(condition), scales = "free", space="free")+
