...
for (i in 1:nrow(MIC_mod_av)) { #vertical 
  candidates <- vector()
     for(j in 3:14) {#horizontal 
       if (!is.na(MIC_mod_av[i,j]) & MIC_mod_av[i,j]/MIC_mod_av[i, "max"]>0.25 & MIC_mod_av[i,j]>0.0336) {#-> MIC_value_absolut)
...
for (i in 1:nrow(MBC_mod_av)) { #vertical 
  candidates <- vector()
  for(j in 3:14) {#horizontal 
    if (!is.na(MBC_mod_av[i,j]) & MBC_mod_av[i,j]/MBC_mod_av[i, "max"]>0.25 & MBC_mod_av[i,j]>0.0336) {#-> MIC_value_absolut)
...
order<-aggregate(df_final_replicates$MBC, by=list(df_final_replicates$bacteria), median, na.rm=T)
...
MBC_MIC_rel_long_label%>%
 mutate(bacteria = recode(bacteria, "Eubacterium eligens" = "Lachnospira eligens" ,
                          ...
                          "Lachnospira sp" = "Lachnospiraceae bacterium"))->MBC_MIC_rel_long_label
