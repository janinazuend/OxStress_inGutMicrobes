...
write.xlsx(tolerances_h2o2, "P:/Shared_documents/Janina_Zuend/08_phd/12_working_package_2/01_OD_pure/02_H2O2/H2O2_tolerances.xlsx")
```
...
sub%>%
 ...
 geom_raster(data = filter(sub, assay == "MBC"), aes(fill = OD)) +
  ...
  geom_point(data = subset(sub, tolerance == condition & assay %in% "a_MIC"), aes(x = tolerance, y = assay), shape=16, colour="seagreen2", size=2)+
   geom_point(data = subset(sub, condition == tolerance+1 ), aes(x = condition, y = assay), shape=16, colour="indianred", size=2)+
  geom_point(data = subset(sub, condition == tolerance+1 ), aes(x = condition, y = assay), shape=16, colour="indianred", size=2)->growth_h2o2
