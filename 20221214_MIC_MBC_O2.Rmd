...
colnames(MIC_replicates)<-list("bacteria", 1, 2.2, 4.7, 10.3, 22.4, 48.85, 106.3, 231.4, 503.5, 1095.9 , 2385.2 , 5191.4, 11299.08 )

MBC_replicates <-data.frame(row_names, MBC_values)
colnames(MBC_replicates)<-list("bacteria", 1, 2.2, 4.7, 10.3, 22.4, 48.85, 106.3, 231.4, 503.5, 1095.9, 2385.2, 5191.4, 11299.08 )
```
...
for (i in 1:nrow(MIC_mod_av)) { #vertical 
  candidates <- vector()
     for(j in 3:14) {#horizontal 
       if (!is.na(MIC_mod_av[i,j]) & MIC_mod_av[i,j]/MIC_mod_av[i, "max"]>0.1 & MIC_mod_av[i,j]>0.03449526) {#-> MIC_value_absolut)
...
for (i in 1:nrow(MBC_mod_av)) { #vertical 
  candidates <- vector()
  for(j in 3:14) {#horizontal 
    if (!is.na(MBC_mod_av[i,j]) & MBC_mod_av[i,j]/MBC_mod_av[i, "max"]>0.1 & MBC_mod_av[i,j]>0.03392658) {#-> MIC_value_absolut)
...
for (i in 1:nrow(MIC_mod_av)) { #vertical 
  candidates <- vector()
     for(j in 3:14) {#horizontal 
       if (!is.na(MIC_mod_av[i,j]) & MIC_mod_av[i,j]/MIC_mod_av[i, "max"]>0.5 & MIC_mod_av[i,j]>0.03449526) {#-> MIC_value_absolut)
...
for (i in 1:nrow(MBC_mod_av)) { #vertical 
  candidates <- vector()
  for(j in 3:14) {#horizontal 
    if (!is.na(MBC_mod_av[i,j]) & MBC_mod_av[i,j]/MBC_mod_av[i, "max"]>0.5 & MBC_mod_av[i,j]>0.03392658 ) {#-> MIC_value_absolut)
...
df_pvalue<-matrix(0, nrow(MIC_replicates)/6 , 12)
  
row=1
for (i in 1:(nrow(MIC_replicates)/6)) { #vertical 
  
  df_pvalue[i,1]<-MIC_replicates[row, 1]

     for(j in 4:14){#horizontal 
       
       control<-MIC_replicates[c(row:(row+5)), c(3)]
       test<-MIC_replicates[c(row:(row+5)), c(j)]
       # print(test)
       
       my_data <- data.frame( 
                group = rep(c("control", "test"), each = 6),
                OD = c(control,  test)
                )
       res <- wilcox_test(my_data, OD ~ group, alternative = "greater")
...
df_pvalue_MBC<-matrix(0, nrow(MBC_replicates)/6 , 12)
  
row=1
for (i in 1:(nrow(MBC_replicates)/6)) { #vertical 
  
  df_pvalue_MBC[i,1]<-MBC_replicates[row, 1]

     for(j in 4:14){#horizontal 
       
       control<-MBC_replicates[c(row:(row+5)), c(3)]
       test<-MBC_replicates[c(row:(row+5)), c(j)]
       # print(test)
...
df_pvalue_both%>%
  gather(condition, p_value, ("2":"12"))->p_long

p_long$p_value<-as.numeric(p_long$p_value)


for (i in 1:nrow(p_long)){
if(p_long[i,c("p_value")]<0.01){
  p_long[i,c("significance")]<-"*"
}}
...
order<-aggregate(df_final_replicates$MBC_75, by=list(df_final_replicates$bacteria), mean)
...
MBC_MIC_rel_long_label$OD[MBC_MIC_rel_long_label$condition == 13]<-NA
