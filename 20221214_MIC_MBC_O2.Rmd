...
colnames(MIC_replicates)<-list("bacteria",  2.2, 4.7, 10.3, 22.4, 48.85, 106.3, 231.4, 503.5, 1095.9, 2385.2, 5191.4, )

MBC_replicates <-data.frame(row_names, MBC_values)
colnames(MBC_replicates)<-list("bacteria", 2.2, 4.7, 10.3, 22.4, 48.85, 106.3, 231.4, 503.5, 1095.9, 2385.2, 5191.4, )
...
df_long$range[df_long$value== " 11298.8"]<-"out_range"
...
library(stats)
hoi<-as.list(sub[,c(2,3)])

nls(OD ~ a/(1 + exp(-b * (condition - c))), data=hoi , start = list(a = -1, b = 1, c = 0)) -> fit

sub %>%
  ggplot(aes(x=condition, y=OD)) +
  geom_point() +
  geom_line(aes(x=condition, y=predict(fit)), color="red")
