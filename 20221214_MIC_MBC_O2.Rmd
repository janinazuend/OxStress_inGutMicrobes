...
average<-aggregate(MIC_mod$`2.2`, list(MIC_mod$bacteria), FUN=mean, na.rm=TRUE, na.action=NULL)
names(average)<-c("bacteria", "average")
MIC_mod_av<-join(MIC_mod, average)



average<-aggregate(MBC_mod$`2.2`, list(MBC_mod$bacteria), FUN=mean, na.rm=TRUE, na.action=NULL)
names(average)<-c("bacteria", "average")
MBC_mod_av<-join(MBC_mod, average)
