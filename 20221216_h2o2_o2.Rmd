...
joined<- join(median_h2o2, median_o2, by=c("bacteria", "assay"))

```

# 2. Correlation of MIC and MBC between H2O2 and O2
```{r}
library(ggpmisc)

joined$assay<-factor(joined$assay, levels=c("MIC", "MBC"))


ggplot(joined, aes(x=value_h2o2, y=value_o2))+
  geom_point(aes(colour=assay))+
  scale_colour_manual(values=c("goldenrod2","cyan4"))+
  geom_text_repel(aes(label=bacteria), size=4)+
  ...
  ylab("Oxigenation scale")+
  guides(colour=guide_legend(title="Assay"))+
...
lacto<-grep("Lactobacillus", joined$bacteria)
...
anaero<-grep("Anaero", joined$bacteria)
bacteria_pyla[anaero,5]="Firmicutes"

anaro<-grep("Anaro", joined$bacteria)
...
anaro<-grep("Anaro", joined$bacteria)
bacteria_pyla[anaro,6]="Clostridia"
...
bacteria_pyla[, c("assay", "bacteria", "value_h2o2", "class")] %>%
  pivot_wider(names_from = assay, values_from = value_h2o2, values_fill = NA) %>%
  ggplot(aes(x=MIC, y=MBC))+
  ...
  geom_jitter(aes(colour = class), width = 0.05, height = 0.05, size=3, alpha=0.5)+
  # scale_colour_manual(values=c("goldenrod2","cyan6"))+
  # geom_text_repel(aes(label=bacteria, colour=class),lineheight= 3,  fontface = 'bold.italic', size=4, point.padding = unit(0.01, "lines"), force= 1, max.overlaps = 1000000, min.segment.length = 0)+
  scale_y_log10()+
  scale_x_log10()+
...
bacteria_pyla[, c("assay", "bacteria", "value_o2", "class")] %>%
  pivot_wider(names_from = assay, values_from = value_o2, values_fill = NA) %>%
  ggplot(aes(x=MIC, y=MBC))+
  stat_ellipse(aes(group=class, fill=class), geom="polygon", alpha=0.2, show.legend = F)+
  # stat_poly_line()+
  geom_abline(slope=1, colour= "darkgrey", size=1, linetype="dashed")+
  # stat_poly_eq(aes(label=c(..rr.label..)), size=5)+
  geom_jitter(aes(colour = class), width = 0.1, height = 0.1, size=3, alpha=0.5)+
  # scale_colour_manual(values=c("goldenrod2","cyan6"))+
  # geom_text_repel(aes(label=bacteria, colour=class),lineheight= 3,  fontface = 'bold.italic', size=4, point.padding = unit(0.01, "lines"), force= 1, max.overlaps = 1000000, min.segment.length = 0)+
  scale_y_log10()+
  scale_x_log10()+
