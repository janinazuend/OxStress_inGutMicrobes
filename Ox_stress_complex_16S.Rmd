...
ggsave(filename = "alpha_cultures.jpeg",
       ...
       units = c("cm"))
```
#alpha comapred to control ==>Final plot
```{r}
...
alphas%>%
  ...
  scale_fill_manual(values=c("skyblue3", "lightpink3"), labels=c("Control", "Stress"))+
 theme(strip.text = element_text(size  = 20),
...
```
#Alpha compared to control ==>Final plot
```{r}
alphas[, c("donor_name", "time", "stress_level", "stress", "diversity_shannon", "incubation")]%>%
  subset(!time %in% "pre-stress")%>%
  pivot_wider(values_from = diversity_shannon, names_from = incubation)%>%
...
for (i in 1:length(time)){
  ...
  phyloseq_rare %>%
  subset_samples(!Sample_id_compete %in% c("feces_dil_1", "feces"))%>%
  subset_samples(stress %in% "O2") ->sub
  ...
  phyloseq::distance(method = "bray") -> bc
    ...
    vegan::adonis2(bc ~ get_variable(sub,"stress_level")  + get_variable(sub,"time") + get_variable(sub,"incubation")+ get_variable(sub,"donor_name") , p.adjust.methods= "BH")->adonis
...
phyloseq_rare %>%
  ...
  pdataframe%>%
      ...
      # scale_color_manual(values=c('darkgoldenrod1', "cyan4"))+
      geom_point(aes(shape=incubation, colour=donor_name), size=4, stroke=1.5)+
    stat_ellipse(aes(group=incubation))+
      theme(axis.text.y= element_text(size = 20), 
          axis.title.y = element_text(size=20), 
          axis.title.x = element_text(size=20),
          axis.text.x = element_text(size=20),
