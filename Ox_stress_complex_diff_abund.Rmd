...
```{r}
ox <- makeTreeSummarizedExperimentFromPhyloseq(phyloseq_nonrare) 
```
...
out[,c(1,3,6,8,10, 14,15,16)]%>%
  subset(Family %in% c("Escherichia-Shigella","Enterobacteriaceae Family", "Bacteroides", "Anaerostipes"))%>%
  subset(V15 %in% "stress")%>%
  mutate(V14 = factor(V14, levels =c("control", "low", "median", "high","max")))%>%
  ggplot(aes(y=diff.btw, x=V14))+
  geom_point()+
...
```{r}
# library(scales)
filter_decreased<-c("Bacteroides", "Fusicatenibacter", "Anaerostipes", "Lachnospira",
               "Blautia", "Agathobacter",
             "Dorea", "Faecalibacterium", "Lachnospiraceae ND3007 group", 
             "Lachnoclostridium", "Lachnospiraceae UCG-004", "Lachnospiraceae Family",
             "[Eubacterium] hallii group",  "Collinsella", "[Ruminococcus] gauvreauii group",
             "Parabacteroides", "Butyricicoccus")

filter_increased<-c("Enterobacteriaceae Family", "Morganella","Escherichia-Shigella", "[Clostridium] innocuum group","Enterococcus", "Bifidobacterium","Hafnia-Obesumbacterium" , "Streptococcus")
...
left_join(all_h2o2_genus[, c("we.ep", "wi.ep", "diff.btw" ,"V14", "V15", "Family", "rab.all")], dir)%>%
   mutate(wi.ep = ifelse(wi.ep < 0.001 , "***", wi.ep))%>%
  mutate(wi.ep = ifelse(wi.ep < 0.05 & wi.ep >= 0.01, "*", wi.ep))%>%
  mutate(wi.ep = ifelse(wi.ep < 0.01 & wi.ep >= 0.001, "**", wi.ep))%>%
 
  mutate(wi.ep = ifelse(!wi.ep == c("*") & !wi.ep == "**" & !wi.ep == "***", NA, wi.ep))%>%
 ...
 scale_fill_gradientn(colors = c( "coral", "white", "cyan4"),  na.value= "grey", values = rescale(c(-12, 0, 12)), limits=c(-12, 12), name="Clr-difference")+
  ...
  geom_text(aes(label=wi.ep), size=5)
